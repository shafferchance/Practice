...
import { FederatedWorker, ModuleReturn } from "federated-worker";
...
function setupFederatedWorker() {
  ...
  const args = getHtmlElementById<HTMLInputElement>("remoteFunctionA");
  const exec = getHtmlElementById<HTMLButtonElement>("remoteFunctionE");

  const remoteESMWorker = new FederatedWorker(true);
  ...
  exec.addEventListener("click", (event) => {
    ...
    const functionArgs = args.value.split(",");

    name.value = "";
    module.value = "";
    args.value = "";

    const result = remoteESMWorker.runMethod(
      {
        method: functionName,
        module: functionModule === "" ? undefined : functionModule,
        args: functionArgs,
      },
      true
    );
